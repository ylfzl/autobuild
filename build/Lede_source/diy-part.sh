...
cat >$NETIP <<-EOF
uci set network.lan.ipaddr='192.168.11.20'                                    # IPv4 地址(openwrt后台地址)
uci set network.lan.netmask='255.255.255.0'                                 # IPv4 子网掩码
uci set network.lan.gateway='192.168.11.254'                                   # IPv4 网关
uci set network.lan.broadcast='192.168.11.255'                               # IPv4 广播
...
sed -i "s/OpenWrt /$Yang compiled in $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" $ZZZ            # 增加个性名字 ${Author} 默认为你的github帐号
