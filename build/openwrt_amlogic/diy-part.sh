...
uci set network.lan.dns='8.8.8.8 114.114.114.114'                         # DNS(多个DNS要用空格分开)
...
sed -i "s/OpenWrt /TomCat Compiled in $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" $ZZZ          # 增加个性名字${Author}默认为你的github账号
